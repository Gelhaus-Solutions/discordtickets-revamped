...
if (!semver.satisfies(process.versions.node, pkg.engines.node)) {
  console.log(
    '\x07' +
      colours.redBright(
        `Error: Your current Node.js version, ${process.versions.node}, does not meet the requirement "${pkg.engines.node}".\n` +
          `Please update to version ${semver.minVersion(pkg.engines.node).version} or higher.`
      )
  );
  process.exit(1);
}
...
const isContainerRuntime = Boolean(process.env.DOCKER || process.env.PTERODACTYL);

// On Docker/Pterodactyl the app code and working directory may intentionally differ.
// Only warn outside container runtimes.
if (base_dir !== cwd && !isContainerRuntime) {
  console.log(
    '\x07' +
      colours.yellowBright('Warning: The current working directory is not the same as the base directory.')
  );
  console.log(
    colours.yellowBright(
      'This may result in unexpected behaviour, particularly with missing environment variables.'
    )
  );
...
// Create a Logger using the default config and set listeners as early as possible.
...
process.on('uncaughtException', (error, origin) => {
  ...
  log.warn(
    origin === 'uncaughtException'
      ? 'Uncaught exception'
      : 'Unhandled promise rejection' + ` (${error.name})`
  );
  log.error(error);
});
