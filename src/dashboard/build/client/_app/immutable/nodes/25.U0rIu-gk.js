import'../chunks/B17Q6ahh.js';import'../chunks/Bzak7iHL.js';import{
	aU as Qt, p as Wt, d as w, U as Gt, i as e, h as Jt, t as L, aV as $, g as t, a as c, b as Xt, c as s, r as i, j as tt, s as d, f as u, aT as at,
}from'../chunks/CWXCXDbJ.js';import{ s as _ }from'../chunks/BTVB6o0Y.js';import{ i as T }from'../chunks/BnKh12PZ.js';import{
	e as Yt, i as Zt,
}from'../chunks/DuqOsHh6.js';import{
	b as ta, r as aa, s as ea,
}from'../chunks/DNgBoiT1.js';import{ b as ra }from'../chunks/C43HmXkP.js';import{
	s as sa, a as ia,
}from'../chunks/DeGpVRd_.js';import{ p as oa }from'../chunks/ckya4THT.js';async function da({
	fetch:et, params:rt,
}){
	try{
		return{
		...
		return console.error('Failed to load transcripts:', I), {
			transcripts:[],
			totalTranscripts:0,
			searched:!1,
		};
	...
	Wt(rt, !0);const I=()=>ia(oa, '$page', mt), [mt, xt]=sa();const A=w(''), E=w('date'), U=w(!1), m=w(!1), S=w(Gt([])), x=w(1), b=w(1), P=w(0), ht=20;const yt=(a, r)=>{
			...
			try{
				const r=await fetch(`/api/admin/guilds/${I().params.guild}/tickets/${a}/transcript`, { credentials:'include' });if(!r.ok)throw new Error('Failed to download transcript');const f=await r.blob(), h=window.URL.createObjectURL(f), g=document.createElement('a');g.href=h, g.download=`transcript-${a}.html`, document.body.appendChild(g), g.click(), window.URL.revokeObjectURL(h), document.body.removeChild(g);
			}catch(r){
				alert('Failed to download transcript: '+r.message);
			}
		};const st=ha(), it=e(Jt(st), 2), M=s(it), N=s(M), ot=e(s(N), 2), wt=s(ot, !0);i(ot), i(N);const z=e(N, 2), dt=e(s(z), 2), St=s(dt);i(dt), i(z);const lt=e(z, 2), nt=e(s(lt), 2), Dt=s(nt);{ const $t=a=>{
			const r=la();c(a, r);
		}, Tt=a=>{
			const r=at('Ready');c(a, r);
		}, At=a=>{
			const r=at('Waiting for search');c(a, r);
		};T(Dt, a=>{
		t(m)?a($t):t(U)?a(Tt, 1):a(At, !1);
	}); }i(nt), i(lt), i(M);const q=e(M, 2), K=s(q), V=s(K), H=e(s(V), 2);aa(H), tt(2), i(V);const ct=e(V, 2), Q=e(s(ct), 2), W=s(Q);W.value=W.__value='date';const vt=e(W);vt.value=vt.__value='duration', i(Q), i(ct), i(K);const C=e(K, 2), Lt=s(C);{ const Pt=a=>{
			const r=na();tt(), c(a, r);
		}, Rt=a=>{
			const r=ca();tt(), c(a, r);
		};T(Lt, a=>{
		t(m)?a(Pt):a(Rt, !1);
	}); }i(C), i(q);const jt=e(q, 2);{ const Ft=a=>{
			const r=va();c(a, r);
		}, It=a=>{
			const r=fa(), f=s(r), h=s(f);i(f);const g=e(f, 2);Yt(g, 17, ()=>t(S), Zt, (D, o)=>{
				const n=ua(), v=s(n), p=s(v), k=s(p), j=e(s(k));i(k);const F=e(k, 2), gt=s(F), Ct=e(gt);{ const Et=l=>{
					const y=ga(), Z=e(s(y));i(y), L(()=>_(Z, ` ${t(o).userId??''}`)), c(l, y);
				};T(Ct, l=>{
					t(o).userId&&l(Et);
				}); }i(F), i(p);const pt=e(p, 2), ut=s(pt), Ot=s(ut);{ const Mt=l=>{
					const y=at();L((Z, Ht)=>_(y, `${Z??''}
									${Ht??''}`), [()=>new Date(t(o).createdAt).toLocaleDateString(), ()=>new Date(t(o).createdAt).toLocaleTimeString([], {
						hour:'2-digit',
						minute:'2-digit',
					})]), c(l, y);
				};T(Ot, l=>{
					t(o).createdAt&&l(Mt);
				}); }i(ut), i(pt), i(v);const J=e(v, 2), X=s(J), Nt=e(s(X));i(X);const Y=e(X, 2), zt=e(s(Y));i(Y);const qt=e(Y, 2);{ const Kt=l=>{
					const y=pa();c(l, y);
				};T(qt, l=>{
					t(o).open||l(Kt);
				}); }i(J);const bt=e(J, 2), ft=s(bt), Vt=e(ft, 2);i(bt), i(n), L((l, y)=>{
					_(j, ` ${(t(o).topic||`Ticket ${t(o).id}`)??''}`), _(gt, `ID: ${t(o).id??''} `), _(Nt, ` Duration: ${l??''}`), _(zt, ` Size: ${y??''} KB`), ea(Vt, 'href', `/transcript/${t(o).id}`);
				}, [()=>yt(t(o).createdAt, t(o).closedAt), ()=>{
					let l;return((((l=t(o).htmlTranscript)==null?void 0:l.length)||0)/1024).toFixed(2);
				}]), $('click', ft, ()=>kt(t(o).id)), c(D, n);
			});const R=e(g, 2);{ const G=D=>{
				const o=ba(), n=s(o), v=e(n, 2), p=e(v, 2), k=s(p);i(p);const j=e(p, 2), F=e(j, 2);i(o), L(()=>{
					n.disabled=t(x)===1||t(m), v.disabled=t(x)===1||t(m), _(k, `Page ${t(x)??''} of ${t(b)??''}`), j.disabled=t(x)===t(b)||t(m), F.disabled=t(x)===t(b)||t(m);
				}), $('click', n, ()=>B(1)), $('click', v, ()=>B(t(x)-1)), $('click', j, ()=>B(t(x)+1)), $('click', F, ()=>B(t(b))), c(D, o);
			};T(R, D=>{
				t(b)>1&&D(G);
			}); }i(r), L(()=>_(h, `Showing page ${t(x)??''} of ${t(b)??''} (${t(S).length??''} results on this page)`)), c(a, r);
		}, Ut=a=>{
			const r=ma();c(a, r);
		}, Bt=a=>{
			const r=xa();c(a, r);
		};T(jt, a=>{
		t(m)?a(Ft):t(S).length>0?a(It, 1):t(U)?a(Ut, 2):a(Bt, !1);
	}); }i(it), L(()=>{
		_(wt, t(P)), _(St, `${t(x)??''} / ${t(b)??''}`), C.disabled=t(m);
	}), $('keydown', H, _t), ra(H, ()=>t(A), a=>d(A, a)), ta(Q, ()=>t(E), a=>d(E, a)), $('click', C, ()=>O(1)), c(et, st), Xt(), xt();
}Qt(['keydown', 'click']);export{
	Ra as component, Pa as universal,
};
