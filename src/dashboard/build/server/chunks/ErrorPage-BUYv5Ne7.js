import {
	k as store_get, f as attr_class, l as unsubscribe_stores, ak as ensure_array_like, al as attr_style, am as stringify,
...
function ErrorPage($$renderer, $$props) {
	$$renderer.component($$renderer2 => {
		...
		Tree_1($$renderer2, { entry: flatten(store_get($$store_subs ??= {}, '$page', page).error) });
		$$renderer2.push('<!----></div> ');
		if (store_get($$store_subs ??= {}, '$page', page).params && Object.keys(store_get($$store_subs ??= {}, '$page', page).params).length > 0) {
			...
			Tree_1($$renderer2, { entry: flatten(store_get($$store_subs ??= {}, '$page', page).params) });
			$$renderer2.push('<!----></div>');
		} else {
			$$renderer2.push('<!--[!-->');
		}
