import {
	ao as attr, ak as ensure_array_like, f as attr_class, al as attr_style, k as store_get, l as unsubscribe_stores,
...
function _page($$renderer, $$props) {
	$$renderer.component($$renderer2 => {
		let $$store_subs;
		const { data } = $$props;
		const sinceDate = new Date(Date.now() - 30 * 24 * 60 * 60 * 1e3);
		...
		if (data.trend && data.trend.length > 0) {
			...
			for (let $$index_2 = 0, $$length = each_array_2.length; $$index_2 < $$length; $$index_2++) {
				const point = each_array_2[$$index_2];
		...
		for (let $$index_5 = 0, $$length = each_array_3.length; $$index_5 < $$length; $$index_5++) {
			const rating = each_array_3[$$index_5];
		...
		if (Object.keys(data.feedbackByCategory || {}).length > 0) {
			...
			for (let $$index_8 = 0, $$length = each_array_6.length; $$index_8 < $$length; $$index_8++) {
				const [categoryName, categoryFeedback] = each_array_6[$$index_8];
				...
				for (let $$index_7 = 0, $$length2 = each_array_8.length; $$index_7 < $$length2; $$index_7++) {
					const rating = each_array_8[$$index_7];
		...
		if ((data.feedback || []).length > 0) {
			...
			for (let $$index_10 = 0, $$length = each_array_9.length; $$index_10 < $$length; $$index_10++) {
				const feedback = each_array_9[$$index_10];
				...
				if (feedback.createdAt) {
					...
					$$renderer2.push(`<div class="text-xs text-gray-500">${escape_html(new Date(feedback.createdAt).toLocaleDateString())} at 
								${escape_html(new Date(feedback.createdAt).toLocaleTimeString([], {
		hour: '2-digit',
		minute: '2-digit',
	}))}</div>`);
				} else {
...
// # sourceMappingURL=_page.svelte-CSHCtnp8.js.map
