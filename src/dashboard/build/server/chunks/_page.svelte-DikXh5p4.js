import {
	k as store_get, l as unsubscribe_stores, ao as attr, ak as ensure_array_like, f as attr_class, am as stringify, j as getContext, ar as bind_props, as as fallback, al as attr_style, at as slot,
...
import {
	g as get, w as writable,
} from './index-PiinLdtd.js';
...
function ToastContainer($$renderer, $$props) {
	$$renderer.component($$renderer2 => {
		let $$store_subs;
		const theme = fallback($$props.theme, 'dark');
		const placement = fallback($$props.placement, 'bottom-right');
		const type = fallback($$props.type, 'info');
		const showProgress = fallback($$props.showProgress, false);
		const duration = fallback($$props.duration, 3e3);
		const width = fallback($$props.width, '320px');
		const placements = [
			'bottom-right',
			'bottom-left',
			'top-right',
		...
		for (let $$index_1 = 0, $$length = each_array.length; $$index_1 < $$length; $$index_1++) {
			const placement2 = each_array[$$index_1];
			...
			for (let $$index = 0, $$length2 = each_array_1.length; $$index < $$length2; $$index++) {
				const toast = each_array_1[$$index];
...
function BootstrapToast($$renderer, $$props) {
	$$renderer.component($$renderer2 => {
		const theme = fallback($$props.theme, 'light');
		const data = fallback($$props.data, () => ({}), true);
...
function _page($$renderer, $$props) {
	$$renderer.component($$renderer2 => {
		...
		function $$render_inner($$renderer3) {
			...
			{
				const children = function($$renderer4, { data: toasted }) {
...
// # sourceMappingURL=_page.svelte-DikXh5p4.js.map
