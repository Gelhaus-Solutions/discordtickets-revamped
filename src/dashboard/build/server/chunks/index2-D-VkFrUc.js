...
function r(e){
	let t, f, n='';if('string'===typeof e||'number'===typeof e){
		n+=e;
	}else if('object'===typeof e){
		if(Array.isArray(e)){
			const o=e.length;for(t=0;t<o;t++)e[t]&&(f=r(e[t]))&&(n&&(n+=' '), n+=f);
		}else {
			for(f in e)e[f]&&(n&&(n+=' '), n+=f);
		}
	}return n;
}function clsx$1(){
	for(var e, t, f=0, n='', o=arguments.length;f<o;f++)(e=arguments[f])&&(t=r(e))&&(n&&(n+=' '), n+=t);return n;
}
...
class Batch {
   ...
   */
	unskip_effect(effect) {
		const tracked = this.#skipped_branches.get(effect);
		if (tracked) {
   ...
   */
	process(root_effects) {
		queued_root_effects = [];
		this.apply();
		const effects = collected_effects = [];
   ...
   */
	#traverse_effect_tree(root, effects, render_effects) {
		...
		while (effect !== null) {
			...
			const skip = is_skippable_branch || (flags & INERT) !== 0 || this.#skipped_branches.has(effect);
   ...
   */
	...
	#commit() {
		if (batches.size > 1) {
			this.previous.clear();
			const previous_batch = current_batch;
			...
			for (const batch of batches) {
				...
				if (others.length > 0) {
					const prev_queued_root_effects = queued_root_effects;
					queued_root_effects = [];
...
function flushSync(fn) {
	const was_flushing_sync = is_flushing_sync;
...
function destroy_derived_effects(derived2) {
	const effects = derived2.effects;
	if (effects !== null) {
...
function update_derived(derived2) {
	const value = execute_derived(derived2);
...
function proxy(value) {
	...
	const is_proxied_array = is_array(value);
	const version = /* @__PURE__ */ state(0);
	const parent_version = update_version;
	const with_parent = fn => {
		if (update_version === parent_version) {
			return fn();
		}
		const reaction = active_reaction;
		const version2 = update_version;
	...
	return new Proxy(
		...
		value,
		{
			defineProperty(_, prop, descriptor) {
				...
				if (s === void 0) {
					with_parent(() => {
						const s2 = /* @__PURE__ */ state(descriptor.value);
						sources.set(prop, s2);
						return s2;
					});
				} else {
			...
			get(target, prop, receiver) {
				...
				if (s === void 0 && (!exists || get_descriptor(target, prop)?.writable)) {
					s = with_parent(() => {
						const p = proxy(exists ? target[prop] : UNINITIALIZED);
						const s2 = /* @__PURE__ */ state(p);
						return s2;
					});
					sources.set(prop, s);
				}
			...
			getOwnPropertyDescriptor(target, prop) {
				...
				if (descriptor && 'value' in descriptor) {
					...
					const source2 = sources.get(prop);
					const value2 = source2?.v;
			...
			has(target, prop) {
				...
				let s = sources.get(prop);
				const has = s !== void 0 && s.v !== UNINITIALIZED || Reflect.has(target, prop);
				if (s !== void 0 || active_effect !== null && (!has || get_descriptor(target, prop)?.writable)) {
					if (s === void 0) {
						s = with_parent(() => {
							const p = has ? proxy(target[prop]) : UNINITIALIZED;
							const s2 = /* @__PURE__ */ state(p);
							return s2;
						});
						sources.set(prop, s);
			...
			set(target, prop, value2, receiver) {
				...
				if (is_proxied_array && prop === 'length') {
					for (let i = value2; i < /** @type {Source<number>} */
          s.v; i += 1) {
				...
				if (s === void 0) {
					...
					const p = with_parent(() => proxy(value2));
					set(s, p);
				}
				const descriptor = Reflect.getOwnPropertyDescriptor(target, prop);
				if (descriptor?.set) {
			...
			ownKeys(target) {
				get(version);
				const own_keys = Reflect.ownKeys(target).filter(key2 => {
					const source3 = sources.get(key2);
					return source3 === void 0 || source3.v !== UNINITIALIZED;
				});
				for (const [key, source2] of sources) {
...
function init_operations() {
	...
	const text_prototype = Text.prototype;
...
function block(fn, flags = 0) {
	const effect = create_effect(BLOCK_EFFECT | flags, fn);
	return effect;
}
function branch(fn) {
...
function destroy_effect(effect, remove_dom = true) {
	...
	const transitions = effect.nodes && effect.nodes.t;
	if (transitions !== null) {
...
function remove_effect_dom(node, end) {
	while (node !== null) {
		const next = node === end ? null : /* @__PURE__ */ get_next_sibling(node);
		node.remove();
		node = next;
	}
}
...
function pause_children(effect, transitions, local) {
	...
	effect.f ^= INERT;
	const t = effect.nodes && effect.nodes.t;
	if (t !== null) {
	...
	while (child !== null) {
		const sibling = child.next;
		const transparent = (child.f & EFFECT_TRANSPARENT) !== 0 || // If this is a branch effect without a block effect parent,
...
function update_reaction(reaction) {
	...
	const previous_update_version = update_version;
	const flags = reaction.f;
  ...
  null;
	...
	try {
		...
		const is_fork = current_batch?.is_fork;
		if (new_deps !== null) {
			var i;
			if (!is_fork) {
...
function get(signal) {
	...
	if (active_reaction !== null && !untracking) {
		const destroyed = active_effect !== null && (active_effect.f & DESTROYED) !== 0;
	...
	if (is_derived) {
		...
		const should_connect = (derived2.f & CONNECTED) === 0 && !untracking && active_reaction !== null && (is_updating_effect || (active_reaction.f & CONNECTED) !== 0);
		const is_new = (derived2.f & REACTION_RAN) === 0;
...
function to_class(value, hash, directives) {
	let classname = value == null ? '' : '' + value;
	if (hash) {
...
function append_styles(styles, important = false) {
	const separator = important ? ' !important;' : ';';
	let css = '';
			...
			for (let i = 0; i < len; i++) {
				...
				if (!in_comment && in_str === false && in_apo === 0) {
					if (c === ':' && name_index === -1) {
						...
						if (name_index !== -1) {
							const name = to_css_name(value.substring(start_index, name_index).trim());
   ...
   */
	async(blockers, fn) {
		...
		if (blockers.length > 0) {
			...
			callback = renderer => Promise.all(blockers).then(() => {
   ...
   */
	static render(component, options = {}) {
		...
		Object.defineProperties(result, {
			html: { get: () => (sync ??= Renderer.#render(component, options)).body },
			head: { get: () => (sync ??= Renderer.#render(component, options)).head },
			body: { get: () => (sync ??= Renderer.#render(component, options)).body },
			hashes: { value: { script: '' } },
			then: {
				value: (
				/**
			...
			if (typeof item === 'string') {
				...
				if (item.#boundary) {
					...
					try {
						...
						const {
							context, failed, transformError,
						} = item.#boundary;
						set_ssr_context(context);
						const transformed = await transformError(error);
...
class SSRState {
   ...
   */
	set_title(value, path) {
		...
		let i = 0;
		const l = Math.min(path.length, current.length);
...
function attr_class(value, hash, directives) {
	const result = to_class(value, hash, directives);
	return result ? ` class="${escape_html(result, true)}"` : '';
}
function attr_style(value, directives) {
	const result = to_style(value, directives);
...
export {
	component_context as $, clear_text_content as A, component_root as B, COMMENT_NODE as C, DevalueError as D, create_text as E, array_from as F, is_passive_event as G, HYDRATION_START as H, set_active_reaction as I, set_active_effect as J, active_reaction as K, LEGACY_PROPS as L, active_effect as M, BOUNDARY_EFFECT as N, block as O, branch as P, queue_micro_task as Q, Batch as R, pause_effect as S, move_effect as T, set_signal_status as U, DIRTY as V, schedule_effect as W, MAYBE_DIRTY as X, defer_effect as Y, set_component_context as Z, handle_error as _, is_plain_object as a, internal_set as a0, destroy_effect as a1, invoke_error_boundary as a2, derived as a3, effect_tracking as a4, render_effect as a5, HYDRATION_END as a6, HYDRATION_START_ELSE as a7, svelte_boundary_reset_onerror as a8, source as a9, untrack as aa, increment as ab, HYDRATION_START_FAILED as ac, push$1 as ad, pop$1 as ae, EFFECT_TRANSPARENT as af, EFFECT_PRESERVED as ag, noop as ah, subscribe_to_store as ai, safe_not_equal as aj, ensure_array_like as ak, attr_style as al, stringify as am, head as an, attr as ao, await_block as ap, spread_props as aq, bind_props as ar, fallback as as, slot as at, sanitize_props as au, attributes as av, stringify_string as b, setContext as c, dev as d, enumerable_symbols as e, attr_class as f, get_type as g, clsx as h, is_primitive as i, getContext as j, store_get as k, unsubscribe_stores as l, set as m, get as n, flushSync as o, define_property as p, mutable_source as q, render as r, stringify_key as s, init_operations as t, uneval as u, valid_array_indices as v, get_first_child as w, get_next_sibling as x, HYDRATION_ERROR as y, hydration_failed as z,
};
// # sourceMappingURL=index2-D-VkFrUc.js.map
