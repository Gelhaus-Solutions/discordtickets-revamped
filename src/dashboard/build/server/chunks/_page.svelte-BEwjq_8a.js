import { ar as run, an as head, ao as attr, am as stringify, ai as store_get, af as attr_class, aj as unsubscribe_stores, ak as ensure_array_like, ag as clsx } from './index2-BZw6XBxw.js';
import { e as escape_html } from './escaping-CqgfEcN3.js';
import { p as page } from './stores-BG2s9ZXP.js';
import { i as iconFor } from './nodes-EH74Ecid.js';
  ...
  $$renderer.component(($$renderer2) => {
    ...
    if (runs.length === 0) {
      ...
      for (let $$index_1 = 0, $$length = each_array.length; $$index_1 < $$length; $$index_1++) {
        let run2 = each_array[$$index_1];
        const status = STATUS[run2.status] ?? STATUS.FAILED;
        $$renderer2.push(`<div class="rounded-xl bg-gray-100/50 p-3 dark:bg-slate-800/50"><button type="button" class="flex w-full items-center gap-3 text-left"><span${attr_class(`rounded-full px-2 py-0.5 text-xs font-medium ${stringify(status.class)}`)}><i${attr_class(`fa-solid ${stringify(status.icon)}`)}></i> ${escape_html(status.text)}</span> <span class="min-w-0 flex-1 truncate text-sm">${escape_html(labelFor(run2.triggerType))}</span> `);
        if (run2.durationMs != null) {
          $$renderer2.push("<!--[-->");
          $$renderer2.push(`<span class="text-xs text-gray-500 dark:text-slate-400">${escape_html(run2.durationMs)}ms</span>`);
        } else {
          $$renderer2.push("<!--[!-->");
        }
        $$renderer2.push(`<!--]--> <span class="text-xs text-gray-400 dark:text-slate-500">${escape_html(new Date(run2.createdAt).toLocaleString())}</span> <i${attr_class(`fa-solid ${stringify(expanded === run2.id ? "fa-angle-up" : "fa-angle-down")}`)}></i></button> `);
        if (run2.error) {
          $$renderer2.push("<!--[-->");
          $$renderer2.push(`<p class="mt-2 rounded bg-red-500/10 p-2 font-mono text-xs text-red-600 dark:text-red-400">${escape_html(run2.error)}</p>`);
        } else {
          $$renderer2.push("<!--[!-->");
        }
        $$renderer2.push(`<!--]--> `);
        if (expanded === run2.id) {
          ...
          const each_array_1 = ensure_array_like(run2.steps ?? []);
...
//# sourceMappingURL=_page.svelte-BEwjq_8a.js.map
