import { ah as getContext, ai as store_get, aj as unsubscribe_stores, F as setContext, an as head, av as ssr_context, _ as derived, ao as attr, af as attr_class, ag as clsx, al as attr_style, aw as attributes, am as stringify, ax as to_array, ay as hasContext, ak as ensure_array_like, ar as invalid_default_snippet, au as bind_props, az as run, aA as is_array, aB as get_prototype_of, aC as object_prototype, aD as exclude_from_object, aq as spread_props, aE as css_props, as as fallback } from './index2-B7lo9Ma0.js';
...
import { G as GRAPH_VERSION, C as CATEGORY_META, i as iconFor, a as CATEGORY_ORDER, c as categoryOf, h as humanDuration, p as parseDuration, s as summarise } from './nodes-BUCpBvSB.js';
import { v as v4, E as EmojiPicker } from './EmojiPicker-7yzMrmFf.js';
...
function BaseNode($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<!--]--></div> <p class="mt-1 truncate text-sm text-gray-500 dark:text-slate-400">${escape_html(summarise({ params: data.params, type: data.type }, state.catalogue))}</p> <span${attr_class(`mt-2 inline-block rounded-full px-2 py-0.5 text-xs font-medium ${stringify(skin().chip)}`)}>${escape_html(skin().label.replace(/s$/, ""))}</span></div> <!--[-->`);
    ...
    for (let i = 0, $$length = each_array.length; i < $$length; i++) {
      ...
      Handle($$renderer2, {
        ...
        class: `!h-3 !w-3 !border-2 !border-white dark:!border-slate-800 ${stringify(handle === "true" ? "!bg-green-500" : handle === "false" ? "!bg-red-400" : skin().handle)}`
      });
      $$renderer2.push(`<!----> `);
      if (outputs().length > 1) {
        $$renderer2.push("<!--[-->");
        $$renderer2.push(`<span${attr_class(`pointer-events-none absolute -right-11 text-[10px] font-semibold ${stringify(handle === "true" ? "text-green-600 dark:text-green-400" : "text-red-500 dark:text-red-400")}`)}${attr_style(`top: calc(${stringify(38 + i * 24)}% - 7px)`)}>${escape_html(handle)}</span>`);
      } else {
        $$renderer2.push("<!--[!-->");
      }
      $$renderer2.push(`<!--]-->`);
...
function DurationField($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    let { field, value, onchange } = $$props;
    let text = value == null ? "" : humanShorthand(value);
...
function RoleField($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    if (multiple) {
      ...
      const each_array = ensure_array_like(state.roles);
      for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
      ...
      $$renderer2.select(
        ...
        ($$renderer3) => {
          ...
          $$renderer3.push(`<!--[-->`);
          const each_array_1 = ensure_array_like(state.roles);
...
function TicketCategoryField($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    if (multiple) {
      ...
      const each_array = ensure_array_like(state.categories);
      for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
      ...
      $$renderer2.select(
        ...
        ($$renderer3) => {
          ...
          $$renderer3.push(`<!--[-->`);
          const each_array_1 = ensure_array_like(state.categories);
          for (let $$index_1 = 0, $$length = each_array_1.length; $$index_1 < $$length; $$index_1++) {
      ...
      if (clause.field === "ticket.answer") {
        ...
        $$renderer2.select(
          ...
          ($$renderer3) => {
            ...
            $$renderer3.push(`<!--[-->`);
            const each_array_3 = ensure_array_like(state.questions);
...
function ParamFields($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    const optionsFor = (field) => field.type === "subject" ? state.catalogue?.subjects ?? [] : field.options ?? [];
...
function NodePalette($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    const state = editorState();
    let search = store_get($$store_subs ??= {}, "$state", state)("");
...
function Toolbar($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    const state = editorState();
    useSvelteFlow();
    let open = store_get($$store_subs ??= {}, "$state", state)(false);
    ...
    if (problems.length > 0) {
      ...
      if (open) {
        $$renderer2.push("<!--[-->");
        $$renderer2.push(`<div class="absolute z-20 mt-1 max-h-64 w-80 overflow-y-auto rounded-xl bg-white p-2 shadow-lg dark:bg-slate-800"><!--[-->`);
        const each_array = ensure_array_like([...errors(), ...warnings()]);
        for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
          let problem = each_array[$$index];
          $$renderer2.push(`<button type="button" class="block w-full rounded-lg p-2 text-left text-xs transition duration-200 hover:bg-gray-100 dark:hover:bg-slate-700"><i${attr_class(`fa-solid ${stringify(problem.severity === "error" ? "fa-circle-xmark text-red-500" : "fa-circle-exclamation text-amber-500")}`)}></i> ${escape_html(problem.message)}</button>`);
        }
        $$renderer2.push(`<!--]--></div>`);
      } else {
        $$renderer2.push("<!--[!-->");
...
const needsLayout = (graph) => (graph?.nodes ?? []).some((n) => !Number.isFinite(n.position?.x) || !Number.isFinite(n.position?.y));
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    let nodes = store_get($$store_subs ??= {}, "$state", state).raw(initial.nodes);
    let edges = store_get($$store_subs ??= {}, "$state", state).raw(initial.edges);
    let name = store_get($$store_subs ??= {}, "$state", state)(data.automation?.name ?? "New automation");
    let enabled = store_get($$store_subs ??= {}, "$state", state)(data.automation?.enabled ?? true);
    let error = store_get($$store_subs ??= {}, "$state", state)(null);
    let loading = store_get($$store_subs ??= {}, "$state", state)(false);
    let testing = store_get($$store_subs ??= {}, "$state", state)(false);
    let testResult = store_get($$store_subs ??= {}, "$state", state)(null);
    ...
    store_get($$store_subs ??= {}, "$state", state)(JSON.stringify({ enabled, graph: starting, name }));
    ...
    function $$render_inner($$renderer3) {
      ...
      SvelteFlowProvider($$renderer3, {
        children: ($$renderer4) => {
          ...
          if (!data.isNew) {
            $$renderer4.push("<!--[-->");
            $$renderer4.push(`<button type="button"${attr("disabled", testing, true)} title="Run the graph without touching Discord, to see which branch it takes" class="rounded-lg bg-gray-200 px-4 py-2 font-medium transition duration-300 hover:bg-gray-300 disabled:opacity-50 dark:bg-slate-600 dark:hover:bg-slate-500"><i${attr_class(`fa-solid ${stringify(testing ? "fa-spinner animate-spin" : "fa-flask")}`)}></i> Test</button>`);
          } else {
            $$renderer4.push("<!--[!-->");
          }
          $$renderer4.push(`<!--]--> <button type="button"${attr("disabled", loading || blocking().length > 0, true)} class="rounded-lg bg-green-300 px-4 py-2 font-medium transition duration-300 hover:bg-green-400 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-green-500/75 dark:hover:bg-green-500"><i${attr_class(`fa-solid ${stringify(loading ? "fa-spinner animate-spin" : "fa-floppy-disk")}`)}></i> Save</button></div> <div class="mb-2">`);
          Toolbar($$renderer4, { problems: state.problems });
          $$renderer4.push(`<!----></div> `);
          if (testResult) {
            $$renderer4.push("<!--[-->");
            $$renderer4.push(`<div class="mb-2 rounded-xl bg-white p-3 text-sm shadow-sm dark:bg-slate-700"><p class="font-medium">Dry run: ${escape_html(testResult.status)} <span class="text-gray-500 dark:text-slate-400">(${escape_html(testResult.durationMs)}ms)</span></p> <p class="text-xs text-gray-500 dark:text-slate-400">Conditions were evaluated for real; actions did nothing.</p> `);
            if (testResult.error) {
              $$renderer4.push("<!--[-->");
              $$renderer4.push(`<p class="mt-1 font-mono text-xs text-red-500">${escape_html(testResult.error)}</p>`);
            } else {
              $$renderer4.push("<!--[!-->");
            }
            $$renderer4.push(`<!--]--> <div class="mt-2 flex flex-wrap gap-1"><!--[-->`);
            const each_array = ensure_array_like(testResult.steps);
            for (let i = 0, $$length = each_array.length; i < $$length; i++) {
              let step = each_array[i];
              $$renderer4.push(`<span class="rounded bg-gray-100 px-2 py-0.5 font-mono text-xs dark:bg-slate-800">${escape_html(step.t)}<span class="text-gray-400">:${escape_html(step.s)}</span></span>`);
            }
          ...
          NodePalette($$renderer4, { hasTrigger: hasTrigger() });
...
//# sourceMappingURL=_page.svelte-qsZhsN1h.js.map
