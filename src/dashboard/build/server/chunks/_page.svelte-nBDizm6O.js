import {
	ao as attr, am as stringify, j as getContext, ak as ensure_array_like, a3 as derived, f as attr_class, ar as bind_props,
...
function _objectSpread2(e) {
	for (let r = 1; r < arguments.length; r++) {
		...
		r % 2 ? ownKeys(Object(t), true).forEach(r => {
...
const plugins = [];
const defaults = { initializeByDefault: true };
const PluginManager = {
...
Sortable.prototype = /** @lends Sortable.prototype */{
	...
	_nulling: function _nulling() {
		...
		const el = this.el;
		savedInputChecked.forEach(checkEl => {
			...
			$$renderer2.select(
				...
				$$renderer3 => {
					...
					for (let $$index_2 = 0, $$length = each_array_2.length; $$index_2 < $$length; $$index_2++) {
						const cat = each_array_2[$$index_2];
		...
		$$renderer2.push('</textarea></label> <!---->');
		{
			$$renderer2.push('<!---->');
			{
				if (category.openingMessage) {
					$$renderer2.push('<!--[-->');
					$$renderer2.push(`<p class="mb-1 mt-2 text-sm font-semibold">Preview</p> <discord-messages${attr('no-background', true)}${attr('light-theme', data.theme !== 'dark')} class="bloc w-full border-0"><discord-message${attr('author', data.client.username)}${attr('avatar', data.client.avatar)}${attr('bot', true)}${attr('timestamp', `Today at ${(/* @__PURE__ */ new Date()).toLocaleTimeString('default', {
						hour: 'numeric',
						minute: 'numeric',
					})}`)} class="py-2" highlight="">`);
					if (category.pingRoles?.length > 0) {
						$$renderer2.push('<!--[-->');
						$$renderer2.push('<!--[-->');
						const each_array_3 = ensure_array_like(category.pingRoles);
						for (let index = 0, $$length = each_array_3.length; index < $$length; index++) {
							const id = each_array_3[index];
		...
		$$renderer2.select(
			...
			$$renderer3 => {
				...
				for (let $$index_4 = 0, $$length = each_array_4.length; $$index_4 < $$length; $$index_4++) {
					const role = each_array_4[$$index_4];
					$$renderer3.option({
						value: role.id,
		...
		$$renderer2.select(
			...
			$$renderer3 => {
				...
				for (let $$index_5 = 0, $$length = each_array_5.length; $$index_5 < $$length; $$index_5++) {
					const slowmode = each_array_5[$$index_5];
					$$renderer3.option({
						value: ms(slowmode) / 1e3,
						class: 'p-1',
					}, $$renderer4 => {
		...
		$$renderer2.select(
			...
			$$renderer3 => {
				...
				for (let $$index_6 = 0, $$length = each_array_6.length; $$index_6 < $$length; $$index_6++) {
					const role = each_array_6[$$index_6];
					$$renderer3.option({
						value: role.id,
						class: 'm-1 rounded p-1',
						style: role._style,
		...
		$$renderer2.select(
			...
			$$renderer3 => {
				...
				for (let $$index_7 = 0, $$length = each_array_7.length; $$index_7 < $$length; $$index_7++) {
					const role = each_array_7[$$index_7];
					$$renderer3.option({
						value: role.id,
						class: 'm-1 rounded p-1',
						style: role._style,
...
// # sourceMappingURL=_page.svelte-nBDizm6O.js.map
