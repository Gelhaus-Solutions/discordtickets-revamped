import {
	ao as attr, ak as ensure_array_like,
	...
	$$renderer.component($$renderer2 => {
		...
		if (searchResults.length > 0) {
			...
			for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
				const transcript = each_array[$$index];
				...
				if (transcript.createdAt) {
					...
					$$renderer2.push(`${escape_html(new Date(transcript.createdAt).toLocaleDateString())}
									${escape_html(new Date(transcript.createdAt).toLocaleTimeString([], {
		hour: '2-digit',
		minute: '2-digit',
	}))}`);
				} else {
...
// # sourceMappingURL=_page.svelte-BV3iUR6u.js.map
