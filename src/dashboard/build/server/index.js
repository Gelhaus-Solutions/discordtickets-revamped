import {
	D as DevalueError, i as is_primitive, g as get_type, a as is_plain_object, e as enumerable_symbols, s as stringify_key, b as stringify_string, v as valid_array_indices, d as dev, u as uneval,
} from './chunks/index2-D-VkFrUc.js';
import {
	H as HttpError, j as json, t as text, S as SvelteKitError, R as Redirect, e as error, A as ActionFailure,
} from './chunks/index-BcOZ6EV9.js';
import {
	w as with_request_store, r as root, d as decode_pathname, n as normalize_path, a as disable_search, b as decode_params, c as resolve, m as make_trackable,
} from './chunks/root-OrB--9Lj.js';
import {
	b as base, a as app_dir, c as assets, r as relative, o as override, d as reset,
} from './chunks/server-Bf8x1V_n.js';
import {
	t as text_decoder, b as base64_decode, v as validate_layout_server_exports, a as validate_layout_exports, c as validate_page_server_exports, d as validate_page_exports, e as text_encoder, g as get_relative_path, f as base64_encode,
} from './chunks/exports-7ECo9oy7.js';
import {
	r as readable, w as writable,
} from './chunks/index-PiinLdtd.js';
...
function static_error_page(options, status, message) {
	const page = options.templates.error({
		status,
...
async function get_hooks() {
	...
	} = await import('./chunks/hooks.server-D7jQdiZY.js'));
	let reroute;
	let transport;
	return {
		handle,
...
function parseSetCookie(input, options) {
	...
	if (!options.map) {
		...
		const cookies = createNullObj();
		return input.reduce((cookies, str) => {
...
`;
			return {
				...
				chunks: promise_id > 1 ? iterator.iterate(({
					index, str,
				}) => {
...
async function load_server_data({
	event, event_state, state, node, parent,
}) {
	...
	if (page_config.ssr) {
		...
		try {
			...
			rendered = await with_request_store({
				...
				const {
					head: head2, html: html2, css, hashes,
				} = (
   ...
   */
	#get_option(option) {
		return this.data.reduce(
			(value, node) => node?.universal?.[option] ?? node?.server?.[option] ?? value,
...
async function render_data(event, event_state, route, options2, manifest, state, invalidated_data_nodes, trailing_slash) {
	...
	try {
		...
		const functions = node_ids.map((n, i) => once(async () => {
			try {
				if (aborted) {
					return (
...
function get_cookies(request, url) {
     ...
     */
		get(name, opts) {
			const best_match = Array.from(new_cookies.values()).filter(c => c.name === name && domain_matches(url.hostname, c.options.domain) && path_matches(url.pathname, c.options.path)).sort((a, b) => b.options.path.length - a.options.path.length)[0];
			if (best_match) {
...
function create_fetch({
	event, options: options2, manifest, state, get_cookie_header, set_internal,
}) {
			...
			fetch: async (info2, init3) => {
				...
				if (set_cookie) {
					for (const str of splitCookiesString(set_cookie)) {
						const {
							name, value, ...options3
						} = parseString(str, { decodeValues: false });
...
async function internal_respond(request, options2, manifest, state) {
	...
	const headers2 = {};
	const {
		cookies, new_cookies, get_cookie_header, set_internal, set_trailing_slash,
	} = get_cookies(
		request,
		url,
	);
			...
			fn: async root_span => {
				...
				return await with_request_store(
					...
					() => options2.hooks.handle({
						event: traced_event,
						resolve: (event2, opts) => record_span({
							name: 'sveltekit.resolve',
							attributes: { 'http.route': event2.route.id || 'unknown' },
							fn: resolve_span => with_request_store(
								null,
...
class Server {
   ...
   */
	async init({
		...
		const {
			env_public_prefix, env_private_prefix,
		} = this.#options;
